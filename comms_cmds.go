...
// buyFromHarbor handles buying resources from a harbor.
// The ship must be at the harbor location.
// The harbor must produce the resource (Production < 0).
// Returns error if validation fails or transaction cannot be completed.
func buyFromHarbor(g *Game, p *Player, ship *Ship, harbor *Harbor, resourceId ResourceType, amount int) error {
	// TODO: Implement buying logic from harbor
	// 1. Validate harbor produces this resource (harbor.Production.Resource(resourceId) < 0)
	// 2. Calculate price: price() function with harbor storage
	// 3. Validate ship has enough gold
	// 4. Validate ship has enough cargo space
	// 5. Transfer resources: harbor -> ship
	// 6. Deduct gold from ship
	// 7. Update player score (p.Score.newPurchase())
	// 8. Update player statistics (p.Statistics.newPurchase())
	panic("buyFromHarbor: not implemented")
}

// sellToHarbor handles selling resources to a harbor.
// The ship must be at the harbor location.
// The harbor must consume the resource (Production > 0).
// Returns error if validation fails or transaction cannot be completed.
func sellToHarbor(g *Game, p *Player, ship *Ship, harbor *Harbor, resourceId ResourceType, amount int) error {
	// TODO: Implement selling logic to harbor
	// 1. Validate harbor consumes this resource (harbor.Production.Resource(resourceId) > 0)
	// 2. Calculate price: price() function with harbor storage
	// 3. Validate ship has enough resources
	// 4. Transfer resources: ship -> harbor
	// 5. Add gold to ship
	// 6. Update player score (p.Score.newSell(), p.Score.newGoldEarned())
	// 7. Update player statistics (p.Statistics.newSell())
	panic("sellToHarbor: not implemented")
}
